...
if [ "$(cat /etc/pivpn/NO_UFW)" -eq 1 ]; then
    ...
    if iptables -t nat -C POSTROUTING -s 10.43.8.0/24 -o "${IPv4dev}" -j MASQUERADE &> /dev/null; then
        ...
        if [[ ${REPLY} =~ ^[Yy]$ ]]; then
            iptables -t nat -F
            iptables -t nat -I POSTROUTING -s 10.43.8.0/24 -o "${IPv4dev}" -j MASQUERADE
    ...
    if iptables -C ufw-user-forward -i tun0 -o "${IPv4dev}" -s 10.43.8.0/24 -j ACCEPT &> /dev/null; then
        echo ":: [OK] Ufw forwarding rule set"
    else
        ERR=1
        read -r -p ":: [ERR] Ufw forwarding rule is not set, attempt fix now? [Y/n] " REPLY
        if [[ ${REPLY} =~ ^[Yy]$ ]]; then
            ufw route insert 1 allow in on tun0 from 10.43.8.0/24 out on "$IPv4dev" to any
            ufw reload
            echo "Done"
        fi
    fi
